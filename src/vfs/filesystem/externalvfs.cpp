 ...
 *
 */
size_t ExternalFiles::getFile(const char *fname, const char* mode)
{
...
/**
 */
void ExternalFiles::releaseFile(size_t file)
{
    fclose(openfiles[file].file);
    openfiles.erase(file);
}

Uint32 ExternalFiles::readByte(size_t file, Uint8 *databuf, Uint32 numBytes)
{
    return fread(databuf, 1, numBytes, openfiles[file].file);
}

Uint32 ExternalFiles::readWord(size_t file, Uint16 *databuf, Uint32 numWords)
{
    Uint32 numRead;
...
/**
 */
Uint32 ExternalFiles::readThree(size_t file, Uint32 *databuf, Uint32 numThrees)
{
    Uint32 numRead;
#if SDL_BYTEORDER == SDL_BIG_ENDIAN
...
Uint32 ExternalFiles::readDWord(size_t file, Uint32 *databuf, Uint32 numDWords)
{
    Uint32 numRead;
#if SDL_BYTEORDER == SDL_BIG_ENDIAN
...
char*  ExternalFiles::readLine(size_t file, char *databuf, Uint32 buflen)	
{
	std::cout << "openfile" << openfiles[file].path << std::endl;
	return fgets(databuf, buflen, openfiles[file].file);
}

Uint32 ExternalFiles::writeByte(size_t file, const Uint8* databuf, Uint32 numBytes)
{
...
/**
 */
Uint32 ExternalFiles::writeWord(size_t file, const Uint16 *databuf, Uint32 numWords)
{
    Uint32 numWrote;
#if SDL_BYTEORDER == SDL_BIG_ENDIAN
...
/**
 */
Uint32 ExternalFiles::writeThree(size_t file, const Uint32 *databuf, Uint32 numThrees)
{
    Uint32 numWrote;
#if SDL_BYTEORDER == SDL_BIG_ENDIAN
...
Uint32 ExternalFiles::writeDWord(size_t file, const Uint32 *databuf, Uint32 numDWords)
{
    Uint32 numWrote;
...
void ExternalFiles::writeLine(size_t file, const char *databuf)
{
    fputs(databuf, openfiles[file].file);
}

int ExternalFiles::vfs_printf(size_t file, const char* fmt, va_list ap)
{
    int ret;
...
void ExternalFiles::flush(size_t file) 
{
    fflush(openfiles[file].file);
}

void ExternalFiles::seekSet(size_t file, Uint32 pos)
{
    fseek(openfiles[file].file, pos, SEEK_SET);
}

void ExternalFiles::seekCur(size_t file, Sint32 pos)
{
    fseek(openfiles[file].file, pos, SEEK_CUR);
}

Uint32 ExternalFiles::getPos(size_t file) const
{
...
Uint32 ExternalFiles::getSize(size_t file) const 
{
    // @todo Abstract this const implementation of operator[].
    openfiles_t::const_iterator i = openfiles.find(file);
    if (openfiles.end() != i) {
...
const char* ExternalFiles::getPath(size_t file) const 
{
    // @todo Abstract this const implementation of operator[].
    openfiles_t::const_iterator i = openfiles.find(file);
    if (openfiles.end() != i) {
...
size_t ExternalFiles::getFile(const char* fname)
{
